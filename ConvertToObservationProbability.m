...
for kk = 1 : number_of_channels
  fqi_cent = 1200 * log2(max(20, inst_freq_map(kk, :)));
  tmpU = erf((repmat(fc_in_cent_U', 1, length(temporal_positions)) - ...
    repmat(fqi_cent, number_of_channels, 1)) ./ ...
    repmat(snr_map(kk, :), number_of_channels, 1) / sqrt(2));
  tmpL = erf((repmat(fc_in_cent_L', 1, length(temporal_positions)) - ...
    repmat(fqi_cent, number_of_channels, 1)) ./ ...
    repmat(snr_map(kk, :), number_of_channels, 1) / sqrt(2));
  tmpProb = (tmpU - tmpL) ./ repmat(sum(tmpU - tmpL, 1), number_of_channels, 1);
  observation_probability_map = observation_probability_map ...
    + repmat(fixed_best_weight(kk, :), number_of_channels, 1) .* tmpProb;
end;
observation_probability_map = observation_probability_map ...
  ./ repmat(sum(observation_probability_map, 1), number_of_channels, 1);
